...
- `naive_bayes/index.html` — prior probability → conditional probability → one feature prediction → independence assumption → N-feature prediction → zero-frequency problem → Laplace smoothing → full NaiveBayes class (init/fit/predict); "Play Tennis" dataset (14 rows, 4 categorical features); bonus: predict_proba, sklearn comparison, continuous features
